...
import { getReplaceImage } from '../../quiz/create-quiz/helper'
...
const QuestionsReview = ({
  ...
  return (
    <QuestionsOptionsItems
      key={`question_${reviewQuestion?.questionId}`}
      style={{ padding: selectedQuestions ? '10px 20px' : '0px' }}
    >
      <QuestionType>
        Question type -{' '}
        {reviewQuestion?.type?.charAt(0).toUpperCase() +
          reviewQuestion?.type?.slice(1)}
      </QuestionType>
      <QuestionsItems>
        <QuestionOptionsItems>
          <h5
            dangerouslySetInnerHTML={{
              __html: wrapEquationsWithSpan(reviewQuestion?.question)
            }}
          />
        </QuestionOptionsItems>
        <Actions>
          ...
          <div style={{ display: 'flex', gap: '20px' }}>
            ...
            {handleOpenQuestions && (
              <AddRemoveButtonV2
                {...{
                  isSelected: addQuestionsOpen?.includes(
                    reviewQuestion?.questionId
                  ),
                  label: 'question',
                  onClick: () => handleOpenQuestions(reviewQuestion?.questionId)
                }}
              />
            )}
          </div>
      ...
      <QuestionOptionContainer>
        {addQuestionsOpen &&
          addQuestionsOpen.includes(reviewQuestion?.questionId) && (
            <>
              {reviewQuestion.options && reviewQuestion.options?.length > 0 && (
                <>
                  <OptionLabel>
                    {' '}
                    <p>Options</p>
                    ...
                    {reviewQuestion?.options?.map(
                      (ptn: OptionDetails, index: any) => (
                        <ChildOptions
                          key={`answer_${index}`}
                          isActive={
                            reviewQuestion.answer.length > 0
                              ? reviewQuestion?.answer?.includes(ptn.number)
                              : ptn.number === reviewQuestion.answer
                          }
                        >
                          <MathJaxWrapper>
                            <h3
                              dangerouslySetInnerHTML={{ __html: ptn?.option }}
                            />
                          </MathJaxWrapper>
                        </ChildOptions>
                      )
              ...
              {reviewQuestion.type && reviewQuestion?.answer && (
                <div className="mt-2">
                  <ExplanationHeading>Correct Answer:</ExplanationHeading>
                  <ExplanationDescription>
                    <p>
                      {reviewQuestion.type === 'multiple_mcq' ||
                      reviewQuestion.type === 'multiple_mcq_any' ? (
                        reviewQuestion?.answer
                          ?.map((item: any) => {
                            return reviewQuestion?.options[item]?.option
                          })
                          .join(', ')
                      ) : reviewQuestion.type === 'tf' ? (
                        String(reviewQuestion?.answer)
                      ) : reviewQuestion.type === 'number-range' ? (
                        reviewQuestion?.answer.join(', ')
                      ) : reviewQuestion.type === 'mcq' ||
                        reviewQuestion.type === 'number' ||
                        reviewQuestion.type === 'assertion' ? (
                        reviewQuestion?.answer + 1
                      ) : reviewQuestion.type === 'fill-blank' ? (
                        reviewQuestion?.answer?.map((item: any) => (
                          <span key={`key_${item}`}>{item + 1}</span>
                        ))
                      ) : (
                        <MarkdownV2>
                          {Number(reviewQuestion?.answer) + 1}
                        </MarkdownV2>
                      )}
                    </p>
                  </ExplanationDescription>
              ...
              {reviewQuestion?.solution && reviewQuestion?.solution && (
                <QuestionsExplanation>
                  ...
                  <ExplanationDescription>
                    <MathJaxWrapper>
                      <p
                        dangerouslySetInnerHTML={{
                          __html: reviewQuestion.solution
                        }}
                      />
                      {/* <MarkdownV2>
                            {ptn?.option
                      ...
                      <p>
                        {reviewQuestion.type === 'mcq' ? (
                          ...
                          reviewQuestion?.answer?.studentAnswer
                            ...
                            .join(', ')
                        ) : reviewQuestion.type !== 'hots' ? (
                          <MarkdownV2>
                            {reviewQuestion?.answer?.studentAnswer?.join(', ')}
                          </MarkdownV2>
                        ) : (
              ...
              {reviewQuestion?.type === 'case' &&
                reviewQuestion.subreviewQuestions &&
                reviewQuestion.subreviewQuestions.length > 0 &&
                reviewQuestion.subQuestions?.map(
                  (subQuestion: any, subInd: number) => (
                      ...
                      <QuestionOptionsItems>
                        <h4>
                          ...
                          <MarkdownV2 maxWidth="250px">
                            ...
                            typeof reviewQuestion.question === 'string'
                              ? getReplaceImage(
