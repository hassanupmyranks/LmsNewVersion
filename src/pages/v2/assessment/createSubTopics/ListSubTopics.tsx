...
const ListSubTopic = () => {
  ...
  const onEditIconClick = useCallback(
    (id: string | number) => {
      history.push(`${ROUTES_V2.EDIT_SUB_TOPICS}/${id}`)
    },
    [history]
  )
    ...
    <PageContainer>
      {isWarning && (
        <WarningPopUp
          ...
          isLoading={isLoading}
          onDelete={() => handleDelete(subTopicId)}
          text="Are you sure you want to delete this Sub-Topic?"
        />
      )}
      <Flex
        alignItems="center"
