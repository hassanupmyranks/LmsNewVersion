...
const SubmittedTestsTable = () => {
  ...
  const AnalyticsList = (id: any, details: any) => {
    const total = details.reduce(
      (acc: any, curr: any) => acc + (curr.totalSubmits || 0),
      0
    )
    if (total > 0) {
      history.push(`${ROUTES_V2.TEACHER_ANALYTICS}?id=${id}`)
    } else {
      CustomToastMessage('Test is not submitted yet', 'error')
    }
  }
  ...
  }, [page, limit, user, selectedGrade.id, searchKey])
  ...
  const columnHelper = createColumnHelper<GetTestsSubmittedData>()
  const columns: ColumnDef<GetTestsSubmittedData, any>[] = [
    ...
    ...(user.role === 'superAdmin'
      ? [
          columnHelper.accessor('institute_details', {
            size: 100,
            header: () => {
              return (
    ...
    ...(user.role === 'superAdmin' || user.role === 'instituteAdmin'
      ? [
          columnHelper.accessor('branch_details', {
            size: 100,
            header: () => {
              return (
                <Flex style={{ marginLeft: '10px' }}>
    ...
    columnHelper.accessor('_id', {
      ...
      cell: (props) => {
        ...
        const isButtonDisabled =
          props?.row?.original?.test_end_time > currentTime
    ...
    <>
      ...
      <PageContainer>
        ...
        <FullHeading>
          ...
          >
            <Heading isActive={true}>Pattern Assessment</Heading>
          </HeadingWrapper>
          <Line />
          <HeadingWrapper
            style={{ width: 'auto' }}
          ...
          >
            <Heading isActive={false}>No Pattern Assessment</Heading>
          </HeadingWrapper>
        </FullHeading>
        <TableWrapper>
          {isLoading && (
...
export default SubmittedTestsTable
